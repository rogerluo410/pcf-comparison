...
sqlStr     = ' select name,version from repo_packages where repo_id =  (select id from repos where product_id = (select id from products where version like \'{0}%\' and deleted = false) and name = \'{1}\' and deleted = false) and deleted = false '.format(options.comparedversion , options.reponame)
print sqlStr
con        = None
oldDict    = {}
newDict    = {}
